...
class TaskViewSet(viewsets.ViewSet):
    ...
    def list_tasks(self, request, pk=None):
        ...
        tasks_with_items = Task.objects.filter(robot_id=robot.id).prefetch_related(
            "item_set"
        )
        ...
        request_body=openapi.Schema(
            ...
            properties={
                "status": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Novo status da tarefa"
                ),
                "started_at": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    format="date-time",
                    description="Data e hora de início da tarefa",
                ),
                "ended_at": openapi.Schema(
                    type=openapi.TYPE_STRING,
                    format="date-time",
                    description="Data e hora de finalização da tarefa",
                ),
                "shift_result": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Resultado da etapa Shift"
                ),
                "sismama_result": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Resultado da etapa Sismama"
                ),
                "stage": openapi.Schema(
                    type=openapi.TYPE_STRING, description="Etapa atual da tarefa"
                ),
            },
            required=[
                "status"
            ],  # Defina quais campos são obrigatórios, neste caso apenas 'status'
        ),
